...
check-codestyle: ## Check Formatting via ISort, Black, darglint.
	...
	poetry run darglint --verbosity 2 vinstaller tests

.PHONY: check-safety
check-safety: ## Check Securty & Safty via Bandit, Safety.
	poetry check
	poetry run safety check --full-report
	poetry run bandit -ll --recursive vinstaller tests
...
test: ## Run Tests & Coverage
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=vinstaller tests/
...
build-pyqt5-resources: ## Build PyQt5 Resources File
	pyrcc5 ./$(IMAGE)/resources.qrc -o ./resources_rc.py

.PHONY: build-pyqt5-ui
build-build-pyqt5-ui: ## Build PyQt5 QtDesigner UI File
	pyuic5 -x ./pyqt5_ui.ui -o ./pyqt5_ui.py  
